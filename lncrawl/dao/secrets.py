...
class Secret(sa.SQLModel, table=True):
    ...
    name: str = sa.Field(primary_key=True, index=True, nullable=False, max_length=255, description="Secret name")
    value: bytes = sa.Field(exclude=True, sa_type=sa.LargeBinary, description="Encrypted secret value")
